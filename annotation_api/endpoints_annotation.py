...
async def train_model(request: TrainModelRequest, background_tasks: BackgroundTasks):
    async def train():
        try:
            ...
            curr_doc = {"project_name": project_name, "start_time": start_time}
            ...
            await asyncio.to_thread(model.train, data=yaml_path, epochs=epochs, imgsz=640, project=models_dir, name=project_name)
...
# @router.get("/read_results")
# async def get_results(input: CSVInput):
#     try:
...
#                             if latest_csv_path:
#                                 await asyncio.to_thread(copy_and_read_results, latest_csv_path)
#                             await asyncio.sleep(10)  # Adjust polling interval as needed
#                         except Exception as e:
#                             print(f"Error in periodic read: {e}")

#             # Run periodic reading in the background
...
#             results = await asyncio.to_thread(copy_and_read_results, latest_csv_path)
...
async def get_results():
    ...
    if ongoing_training is None:
        raise HTTPException(status_code=404, detail=f"Ongoing training not found for project: ")

    latest_project = get_latest_project()
    
    if not latest_project:
        raise HTTPException(status_code=404, detail="No project directories found")
    ...
    return {"Progeress %":(100-progress_percentage[0])}
